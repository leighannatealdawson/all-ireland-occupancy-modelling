...
siteCovs$urban <- rowSums(siteCovs[, c(
        "CLC_111", "CLC_112", "CLC_121", "CLC_122", "CLC_123",
...
siteCovs$forest <- rowSums(siteCovs[, c(
        "CLC_311", "CLC_312", "CLC_313", "CLC_321", "CLC_322",
        "CLC_324", "CLC_332", "CLC_333"
)], na.rm = TRUE)
...
clc_totals_df <- data.frame(
        Cover_Type = names(clc_totals),
        Total = clc_totals
)
...
# Create correlation matrix for main groupings
groupings <- c(
...
# Compute correlation matrix
cor_matrix <- cor(siteCovs[, groupings], use = "complete.obs", method = "spearman")

# Plot correlation matrix with numbers and color
png("correlation_plot_groupings.png", width = 1000, height = 800)
corrplot(cor_matrix,
        method = "color", # Use color shading
        type = "upper", # Show only upper triangle
        addCoef.col = "black", # Add correlation coefficients in black
        ...
        title = "Correlation between Land Cover Groupings",
        mar = c(0, 0, 2, 0)
) # Add margin for title
dev.off()

# Create pairwise scatterplot matrix
png("scatterplot_matrix_groupings.png", width = 1200, height = 1200)
ggpairs(siteCovs[, groupings],
        upper = list(continuous = "cor"), # Show correlation in upper
        lower = list(continuous = "points"), # Show scatterplots in lower
        diag = list(continuous = "densityDiag")
) # Density plots on diagonal
...
if (length(high_cor) > 0) {
        ...
        for (i in 1:nrow(high_cor)) {
                row_name <- rownames(cor_matrix)[high_cor[i, 1]]
                col_name <- colnames(cor_matrix)[high_cor[i, 2]]
