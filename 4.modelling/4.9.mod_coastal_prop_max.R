...
# using coastal propotions calcualted using maximum grid cell szie not proptions of crorine data. 
...
############################ CREATE MODEL 4 #######################################
mod4 <- occu(~ scale(occ) + scale(year) + bait ~ 
...
coef(mod4)
confint(mod4, type = "state", level = 0.95)
...
for (var in covariates_to_plot) {
  ...
  preds <- predict(mod4, newdata = df, type = "state")
...
head(landscape)
landscape <- read.csv("1.data/1.4.final_data_gropupings/landscape_datainput_1km_grid_groupings_27_5_coastal_props.csv")
# set year to mean year of study
landscape$year <- mean(siteCovs$year)

# make predictions of marten psi across landscape
preds_allireland <- predict(mod4, landscape, type = "state")
...
write.csv(irish_grid_preds, "4.modelling/4.modeloutputs/predicted_occupancy_allirelandmod4_coastal_prop.csv", row.names = FALSE)
...
#goodness of fit
occ_gof1 <- mb.gof.test(mod4, nsim = 10, plot.hist = TRUE) 
occ_gof1
