...
siteCovs_dataiunput$urban <- rowSums(buffer[, grep("^CLC_1", names(buffer))], na.rm = TRUE)
...
# change all nas to 0 
siteCovs_dataiunput[is.na(siteCovs_dataiunput)] <- 0

#check for nas 
na_count <- sapply(siteCovs_dataiunput, function(x) sum(is.na(x)))
na_count
