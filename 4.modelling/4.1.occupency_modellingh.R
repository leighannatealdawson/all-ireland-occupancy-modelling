...
siteCovs$urban <- rowSums(siteCovs[, c(
...
groupings <- c(
  "agrinonpas", "pasture", "conifer", "mixedwood",
  "opennoneagri", "urban"
)
...
########################## coniferous forest #######################################
df <- data.frame(cbind(
  conifer = seq(min(siteCovs$conifer), max(siteCovs$conifer), length.out = 200),
...
######################## urban #######################################
df <- data.frame(cbind(
  urban = seq(min(siteCovs$urban), max(siteCovs$urban), length.out = 200),
...
###################### mixedwood #######################################
df <- data.frame(cbind(
  mixedwood = seq(min(siteCovs$mixedwood), max(siteCovs$mixedwood), length.out = 200),
  conifer = mean(siteCovs$conifer),
  agrinonpas = mean(siteCovs$agrinonpas),
  pasture = mean(siteCovs$pasture),
  urban = mean(siteCovs$urban),
...
############################# pasture ################################
df <- data.frame(cbind(
  pasture = seq(min(siteCovs$pasture), max(siteCovs$pasture), length.out = 200),
  conifer = mean(siteCovs$conifer),
  agrinonpas = mean(siteCovs$agrinonpas),
  urban = mean(siteCovs$urban),
  mixedwood = mean(siteCovs$mixedwood),
...
############################ agri non-pasture ###############################
df <- data.frame(cbind(
  agrinonpas = seq(min(siteCovs$agrinonpas), max(siteCovs$agrinonpas), length.out = 200),
  conifer = mean(siteCovs$conifer),
  pasture = mean(siteCovs$pasture),
  urban = mean(siteCovs$urban),
  mixedwood = mean(siteCovs$mixedwood),
...
############################# year ###############################
df <- data.frame(cbind(
  year = seq(min(siteCovs$year), max(siteCovs$year), length.out = 200),
  conifer = mean(siteCovs$conifer),
  pasture = mean(siteCovs$pasture),
  urban = mean(siteCovs$urban),
  mixedwood = mean(siteCovs$mixedwood),
...
# Determine the unique years from the model's training data (umf2@siteCovs)
# These are the year values the model was actually trained on.
# The predict() function will handle scaling internally if the model was fit with scale(year).
distinct_years_from_model_data <- sort(unique(umf2@siteCovs$year))

# Initialize a list to store prediction data frames for each year
all_predictions_list <- list()

# Define base covariate names that are part of the landscape and model
# (excluding 'year', which is handled in the loop)
base_covariate_names <- c("conifer", "pasture", "urban", "mixedwood", "agrinonpas")
grid_id_col_name <- "gridid" # Assuming this is the column name for the grid identifier in your landscape data

# Loop through each distinct year to make predictions
for (current_year in distinct_years_from_model_data) {
  # Create a temporary landscape dataframe specifically for the model's predict function.
  # This only needs the covariates used by the model's state formula.
  temp_landscape_for_pred_model <- landscape[, base_covariate_names] # Select base covariates
  temp_landscape_for_pred_model$year <- current_year # Add/set the current year

  # Make predictions for this specific year using mod1.
  current_preds <- predict(mod1, newdata = temp_landscape_for_pred_model, type = "state") # Returns Predicted, SE, lower, upper

  # Prepare the data frame to combine with predictions for the final output.
  # This will include GRID_ID (if found), the current year, and the other model covariates.
  # Initialize an empty data frame with the correct number of rows to build upon.
  output_df_for_cbind <- data.frame(matrix(ncol = 0, nrow = nrow(landscape)))

  # Add Grid ID column if it exists in the landscape data
  if (grid_id_col_name %in% colnames(landscape)) {
    output_df_for_cbind[[grid_id_col_name]] <- landscape[[grid_id_col_name]]
  } else {
    warning(paste("Grid ID column '", grid_id_col_name, "' not found in landscape data. It will not be included in the output.", sep = ""))
    # If not found, this column will be missing. Consider adding a placeholder if always needed:
    # output_df_for_cbind[[grid_id_col_name]] <- NA
  }

  output_df_for_cbind$year <- current_year # Add the current year

  # Add the other base model covariates directly from the original landscape data
  for (cov_name in base_covariate_names) {
    output_df_for_cbind[[cov_name]] <- landscape[[cov_name]]
  }

  # Combine the constructed landscape/covariate data with the prediction results
  preds_df_year <- cbind(output_df_for_cbind, current_preds)

  # Store in the list
  all_predictions_list[[as.character(current_year)]] <- preds_df_year
}

# Combine all prediction data frames from the list into one
preds_df <- do.call(rbind, all_predictions_list)
rownames(preds_df) <- NULL # Reset row names for a clean data frame
