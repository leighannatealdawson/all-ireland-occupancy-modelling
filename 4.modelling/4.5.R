...
# sanity check in ArcGIS
# Import predicted occupancy for all Ireland
predicted_occupancy <- read.csv("4.modelling/4.modeloutputs/predicted_occupancy_allirelandmod2.csv")

# Import Irish grid 1km with DD for NI
irish_grid <- read.csv("1.data/1.3.processedinarc/irishgrid1kmwithDD.csv")

irish_grid_preds <- merge(irish_grid, predicted_occupancy, by = "gridid", all.x = TRUE)

write.csv(irish_grid_preds, "4.modelling/4.modeloutputs/irish_grid_preds_with_occupancy.csv", row.names = FALSE)
...
# here we have a c-hat of 2.1 - therefore dispersed
