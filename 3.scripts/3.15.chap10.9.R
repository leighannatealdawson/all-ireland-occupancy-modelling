...
#import my data 
data <-read.csv("1.data/1.2.processed/input_data_test1km.csv")
head (data)
View(data)
str(data)
...
elev.orig <- data["built"]      # Unstandardised, original values of covariates
elev.orig <- elev.orig$built
#change nas to 0 
elev.orig[is.na(elev.orig)] <- 0

forest.orig <- data["conifer"]
forest.orig <- forest.orig$conifer
# change any nas to 0 
forest.orig[is.na(forest.orig)] <- 0
View(forest.orig)
nrow_data <-nrow(data)
